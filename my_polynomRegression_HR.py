...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 13 22:47:19 2018
...
def my_poly(arr_train, arr_test, F,N):

    # Data processing into sklearn friendly format 
    train_arr = np.array(arr_train)
    X_test = np.array(arr_test)
...
######################### SIMPLE SOLUTION ##########################################
    ...
    # print('(poly deg 2 + ridge) R-squared score (validation): {:.3f}'
    #      .format(linreg.score(X_val, y_val)))

    pred = linreg.predict(X_test_poly)

    return pred[0] #print(('{:.2f}\n'*len(pred)).format(*pred))
...
if __name__ == '__main__':
    ...
    F,N = int(tmp[0]), int(tmp[1])
    
    for _ in range(0,N):
         arr_train.append(list(map(float, input().rstrip().split())))    
    
    n = int(input().split()[0])
    
    for _ in range(0,n):
        arr_test.append(list(map(float, input().rstrip().split())))    

    result = my_poly(arr_train, arr_test, F, N)
